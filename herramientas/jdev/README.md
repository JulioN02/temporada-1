...
JDEV es un kit de herramientas de desarrollo en linea de comandos (CLI). Tiene 10 utilidades de trabajo diario bajo un solo binario, con dos formas de uso (terminal directa y menu interactivo TUI) y con interfaz de mensajes en español o ingles
...
Para ejecutarlo desde el codigo fuente:

```bash
# opcion A: construir el binario
npm run build
npm link

# opcion B: ejecutar directo desde TypeScript (requiere node >= 22.12)
...
| Forma | Ventaja | Costo |
| --- | --- | --- |
| GUI | Amigable | No automatizable, no composable |
...
**Desarrollo diario:** Generar IDs, validar/configurar JSON, inspecciona JWT, probar APIs, Hashear/verificar contraseñas, limpiar CSV y timestamps.

**Automatizacion y Scripting:** Pipelines de datos (`csv tojson | jq` ), checks de CI (usa los exit codes), verificacion de integridad en releases (`hash` ), generacion de seed (`uuid/password` ), transformacion de exports.

**Seguridad practica:** Contraseñas con **bcrypt** correcto desde el dia 1, tokens leidos sin exponer la firma, requests HTTP con secrets enmascarados.
...
- Orientado a **datos**: la salida es pura (uuids, JSON, hashes…) y se puede **pipesar** (encadenar con `|`, redirigir a archivos, alimentar scripts).
...
- Navegas con flechas / `j` / `k`, Enter para seleccionar.
- 12 opciones: los 9 módulos, **Modo libre** (escribes un comando `jdev` completo a mano), **Idioma (es/en)** y **Salir**.
- El TUI te guía: te pregunta los parámetros uno por uno (versión de UUID, cantidad, archivo, etc.) y muestra los resultados.
- Cambio de idioma **en caliente**: eliges "Idioma" y todo el menú se re-renderiza al instante.
...
| Idiomas | español o inglés (auto-detecta el local del sistema) | sin configuración inicial |
...
Soporta dos versiones segun estandares publicos:

- **V4 (RFC 4122) Aleatorio:** 122 bits generados criptograficamente. La chance de colision es despreciable (se puede generar miles de millones sin miedo)
- **V7 (RFC 9562) Ordenado por tiempo:** Los primeros bits guardan una marca de tiempo, si los UUIDs recien generados se ordenan cronologicamente (perfecto para indices de bases de datos)

#### Usos:

- Claves primarias (evita exponer id secuenciales)
- Nombres de archivos temporales
- IDs de trazas/eventos
- Entidades en APIs

**Capacidades:** Generar en cualquier cantidad, ya sea V4 o V7 como salida limpia una por linea.
...
Dos UUIDs v7 en una sola llamada, uno por linea. Los primeros 12 caracteres hex son la marca de tiempo (ms): como los v7 se generaron en el mismo instante, el prefijo es identico y crece entre llamadas — por eso son ordenables cronologicamente.
...
Sin argumentos devuelve un v4 (el valor es aleatorio y distinto en cada ejecucion).

---

## jdev json - JSON Profesional

Formatear, minificar y validar JSON desde un archivo, un pipe o (en el TUI) por prompts.

JSON es el formato de intercambio de datos estandar (objetos `{"clave": valor}` )
...
**Capacidades:** Posicion exacta del error (linea y columna), entrada por archivo/stdin/flag, todos los errores y/o veredictos de parseo se localizan (validate, format y minify por igual)
...
- `minify` ; Lo compacta a una linea (menos bytes, ideal para APIs/logs)
- `validate` : Chequea que sea JSON valido y devuelve un veredicto
...
| `jdev json format [archivo]` | JSON comprimido → JSON indenado 2 espacios |
| `jdev json minify [archivo]` | JSON indenado → JSON en una línea |
...
JSON comprimido por stdin → formateado con indentacion de 2 espacios en stdout.
...
JSON roto → veredicto localizado con la posicion exacta del error y exit code `2` (error de ejecucion). Un JSON valido responde `valid JSON` con exit `0`.

---

## jdev base64 - Codificacion Base64

Codifica o decodifica texto/binario a base64 (RFC 4648) en variante estandar o URL-safe.
...
- Incrustar imagenes pequeñas en texto

**Capacidades:** Entrada por archivo o stdin, salida binaria exacta al decodificar.

**Limiteaciones:** `jdev base64 decode f > imagen.png` El segundo argumento es una ruta de archivo, no un texto inline. Para codificar un string hay que pipearlo `echo -n "hola" | jdev base64 encode` . Decodificar entrada invalida es error.
...
| `echo -n "aG9sYQ==" \| jdev base64 decode` | decodifica a texto/binary |
...
La variante `--url` usa `-_` y omite el `=` final (valido en query params y JWT).

---

## jdev timestamp - Tiempo UNIX ↔ ISO 8601

Muestra el tiempo actual o convierte epocas (segundos o milisegundos desde `1970-01-01 UTC` ) a fechas legibles y viceversa

El **Epoch Unix** es el numero de segundos (o ms) desde `1970-01-01T00:00:00Z` . La forma en que las maquinas guardan el tiempo
...
Segundos Unix actuales — el formato en el que las maquinas guardan el tiempo.
...
Calcula el hash **SHA-256** (o **SHA-512**) de un archivo o de lo que llegue stdin, en streaming.

Un hash criptografico es una "huella digital" de longitud fija: el mismo contenido produce siempre el mismo hash; cambiar un solo byte produce un hash totalmente distinto.
...
- Verificar que un archivo descargado no se corrompio (`sha256sum` portatil)
...
El fundamento de este modulo es que guardar contraseñas en texto plano es prohibido en produccion. Bycrypt aplica work factor que hace el hash deliberadamente lento y agrega una sal aleatoria (dos hashes de la misma contraseña son distintos). La verificacion es timing-safe (no filtra informacion por velocidad de respuesta)
...
**Limitaciones:** bcrypt usa maximo 72 bytes - jdev rechaza contraseñas que excedan ese limite (por bytes UTF-8, no por caracteres) ANTES de que bcrypt pudiera truncarlas silenciosamente.
...
Hash bcrypt con prefijo `$2b$` (cost 10 por defecto). Cada ejecucion produce un hash **distinto** (sal aleatoria) aunque la clave sea la misma — la verificacion funciona igual.
...
Timing-safe: `Contraseña correcta` (exit `0`) / `Contraseña incorrecta` (exit `2`) — nunca revela mas que eso.
...
Un JWT tiene 3 partes separadas por puntos `header.payload.signature` , cada una en base64url. Se usa para sesiones/autenticacion de APIs.
...
- Inspeccionar que contiene un token (roles, `exp` ,`sub` )
...
RFC 4180 define las reglas: comillas dobles para campos con comas, “” para una comilla literal, CRLF como fin de linea.

jdev lo procesa en streaming, fila por fila con memoria acotada. Puede digerir archivos de 18MB sin problema. Es estricto, texto suelto despues de una comilla de cierre es error (no lo arregla silenciosamente).

#### Usos

- Resuelve la pregunta ¿Cuantas filas tiene este export?
...
HTTP es el protocolo de la web y `curl` es su herramienta clasica. jdev lo que hace es imprimir el **codigo de estado + cabeceras + cuerpo** y por diseño, enmascara las cabeceras sensibles (`authorization`, `cookie`, `set-cookie` ) porque copiar/pegar los con tokens es la forma mas comun de filtrados.
...
Respuesta como texto plano: codigo de estado + cabeceras + cuerpo. Las cabeceras sensibles de la **respuesta** (`authorization`, `cookie`, `set-cookie`) se imprimen como `***` — copiar/pegar tokens es la forma mas comun de filtrarlos. Un 404 tambien sale con exit `0` (el status es dato, no error).

```bash
$ jdev http -X POST -d '{"a":1}' https://api.example.com/echo
```

`-d` manda el body e implica POST; `-H "Authorization: Bearer x"` agrega cabeceras (repetible). El secret se **transmite** al destino (como curl) pero nunca se imprime en pantalla.

---

## Arquitectura e Ingenieria del sistema
...
├── i18n.ts  diccionarios es/en (es = canónico; en tipado contra las mismas
claves)
...
Los codigos de salida son la parte mas importante para scripts

| **CODIGO** | SIGNIFICADO | EJEMPLO |
| --- | --- | --- |
| **0** | Exito (dato o veredicto en stdout) | UUID, HASHES, JSON valido, HTTP aunque sea 404 |
| **1** | Error de **uso** (el comando esta mal escrito/no se puede ejecutar asi) | Flag invalido, accion desconocida, TUI sin TTY |
| **2** | Error de **ejecucion** (la operacion fallo en tiempo de carrera) | Archivo no existe, JSON invalido, bcrypt malformado, timeout de red, verify mismatch |
...
**Datos al stdout, errores al stderr:** Asi `jdev uuid > archivo` jamas contamina el archivo con mensajes.

**Sin color en pipes por defecto:** (`NO_COLOR / FORCE_COLOR` override estandar) la salida es byte exacta y estable

**Newline inteligente:** Los comandos de datos terminan con `\n` ; los binarios seguros (`base64 decode`, `csv tojson`, cuerpo de `http`) escriben verbatim cuando estan pipeados para que quede byte puro y agregan `\n` solo visual en TTY
...
**Contraseñas:** BCRYPT con sal, costo configurable, verificacion timing-safe, guard d 72 bytes medido en bytes UTF-8 (no caracteres) - Aplicado antes de BCRYPT para que nunca trunque en silencio.

**Generacion aleatoria:** Siempre `randomBytes` criptografico, nunca `Math.random` .

**JWT:** La firma jamas se decodifica ni se imprime, los errores no repiten el token.
...
**UUID V4:** Aleatoriedad criptografica

### Notas de seguridad para uso en produccion

- El enmascarado de `jdev http` es de **pantalla**: si pasas `-H "Authorization: Bearer x"`, el header **se transmite** al destino (igual que `curl`) — solo no se imprime en tu terminal.
...
- `--insecure` saltea la verificacion TLS **solo para esa request** — nunca para la sesion global. Usalo solo contra servidores de desarrollo que conoces.

## Dependencias del programa (runtime)

| Dependencia | Versión | Para qué se usa |
| --- | --- | --- |
| `commander` | ^15 | Parseo de argumentos, ayuda automática, códigos de salida estandarizados (es el "motor" del CLI) |
...
- No reemplaza a una GUI: es CLI-first con un TUI de menu.
- Algunos mensajes internos quedan en ingles (errores de uso, help de commander)

## Contribuciones y contacto

- **Bugs, sugerencias y aportes:** abre un issue o un PR en el [repositorio de GitHub](https://github.com/JulioN02/JDEV-TOOL). Para reportar un problema, incluye la salida de `jdev --version` y el comando exacto que fallo.
- **Seguridad:** si encuentras una vulnerabilidad, avisa con detalle. Esta herramienta es de lectura/verificacion por diseño, pero reportar con tiempo siempre ayuda.
- **Autor y contacto:** Julio Nieto (`@JulioN02`). Los commits llevan autoría del proyecto; para preguntas directas, usa los issues para que la respuesta quede publica y util para todos.

## Licencia

[Apache-2.0](https://www.apache.org/licenses/LICENSE-2.0) — © 2026 JulioN02. Puedes usar, copiar, modificar y distribuir la herramienta libremente (incluso con fines comerciales), siempre que conserves el aviso de licencia y atribucion del autor original.